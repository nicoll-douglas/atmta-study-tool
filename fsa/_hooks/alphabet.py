...
def pre_add(new_symbol: str) -> None:
    ...
    if len(new_symbol) != 1:
        raise ValueError("Expected a string of length 1. " f"Got '{new_symbol}'.")


# hook function to run after a symbol is removed from the alphabet of an FSA
def post_discard(symbol: str, current_transition_table: _TransitionTable) -> None:
...
def post_set(
    new_alphabet: AbstractSet[str], current_transition_table: _TransitionTable
) -> None:
    """Remove all transitions from the given transition table if they
